...
simulate.isdm <- function( expected.pop.size=400000, expected.n.PO=300, n.PA=150, n.AA=50, n.DC=50,
                           coefs=list(dist=c(-1,-0.25,0.75), bias=c(-2,-1.5)), DC.pis=matrix( c(0.8,0.76, 0.7,0.73, 0.82,0.67), nrow=3, ncol=2, byrow=TRUE),
                           transect.size = 0.01, #a proportion of cell size.
                           control=list()){
  ...
  Mod1 <- RandomFields::RMgauss( var=1, scale=1) + RandomFields::RMnugget( var=0.01)
  Mod2 <- RandomFields::RMgauss( var=1, scale=1) + RandomFields::RMnugget( var=0.01)
  ...
  dataBrick <- raster::brick( raster::rasterFromXYZ( X[,c(1,2,3)]), raster::rasterFromXYZ( X[,c(1,2,4)]))
  
  #linear predictor
  LinPred <- with( X, model.matrix( ~1+Altitude+Temperature)) %*% coefs$dist
  ...
  if( control$addRandom){
    ...
    REff <- rep( 0, length( LinPred))
  #log-intensity and intensity for log-gauss process
  LinPred <- LinPred + REff
  Intensity <- exp( LinPred)
  #the total expectation and observed number from unbiassed (actual distribution)
  tmp.Lambda <- sum( Intensity)
...
#  Intensity <- Intensity / (ncell( dataBrick) * prod( res( dataBrick)))  #adjusting for cells' area
  
  #Adding these pieces to the dataBrick
  dataBrick <- raster::addLayer( dataBrick, raster::rasterFromXYZ( cbind( X[,1:2], REff=REff)), 
                                 raster::rasterFromXYZ( cbind( X[,1:2], LinPred)), 
                                 raster::rasterFromXYZ( cbind( X[,1:2], Intensity)))
  
  ####  Create the PA data
  #first create the design -- an equal probability spat-bal design
  PAdata <- MBHdesign::quasiSamp( n=n.PA, potential.sites=raster::coordinates(dataBrick))
  
  #now simulate the PA response at those sites (based on point process)
  PAdata$PA <- stats::rbinom( n=n.PA, size=1, prob=1-exp( -raster::raster( dataBrick, "Intensity")[PAdata$ID]*transect.size))
  
  #add the transect area
  PAdata$transectArea <- transect.size * prod( raster::res( dataBrick))
  
  ####  Create the count data (AA)
  AAData <- MBHdesign::quasiSamp( n=n.AA, potential.sites=raster::coordinates(dataBrick))
  
  #now simulate the AA response at those sites (based on point process)
  AAData$AA <- stats::rpois( n=n.AA, lambda=raster::raster( dataBrick, "Intensity")[AAData$ID]*transect.size)
  
  #add the transect area
  AAData$transectArea <- transect.size * prod( raster::res( dataBrick))

  ####  Create the double count data (DC)
  DCData <- MBHdesign::quasiSamp( n=n.DC, potential.sites=raster::coordinates(dataBrick))
  
  #now simulate the DC response at those sites (based on point process)
  #total number of animals available along a transect
  tmpTotCount <- stats::rpois( n=n.DC, lambda=raster::raster( dataBrick, "Intensity")[DCData$ID]*transect.size)
  ...
  maxTmp <- max( xSeq) + max( ySeq)  #sqrt( max( xSeq)^2+max( ySeq)^2)
  dataBrick <- raster::addLayer( dataBrick, raster::rasterFromXYZ( cbind( X[,1:2], dist2City=(maxTmp-rowSums( X[,1:2])) / maxTmp)))

  #probability of being observed (thinning process) -- cloglog link for prob
  dataBrick <- raster::addLayer( dataBrick, 1-exp(- exp( coefs$bias[1]+coefs$bias[2]*raster::raster( dataBrick, "dist2City"))))
  names( dataBrick)[length( names( dataBrick))] <- "obsProb"
  #The biassed intensity after accounting for observation bias.
  dataBrick <- raster::addLayer( dataBrick, biasIntensity=raster::raster( dataBrick, layer="Intensity") * raster::raster( dataBrick, layer="obsProb"))
  names( dataBrick)[length( names( dataBrick))] <- "biasIntensity"

  ####  Take the sample of po data
  N <- stats::rpois( n=1, lambda=expected.n.PO)
  tmp.Lambda <- sum( raster::values( raster::raster( dataBrick, layer="biasIntensity")))
  probs <- raster::raster( dataBrick, layer="biasIntensity") * expected.n.PO / tmp.Lambda  #probably not needed as sample() internally rescales...
  
  sampleID <- sample.int( n=length( raster::values( probs)), size=N, prob=raster::values( probs), replace=TRUE)

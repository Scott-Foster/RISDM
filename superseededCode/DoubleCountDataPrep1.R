...
estimatePisDoubleCount <- function( dat){
  ...
  start.vals <- log( start.vals + 0.001)
...
prepareSingleSurvey <- function( singleDataSet, datasetID, DCcols, survIDname, DCmethod){
  ...
  #the offset for the approximation of log( 1-pi)
  ...
  #should already have label in dataset
  #  singleDataSet <- cbind( singleDataSet, datasetID)
  #  colnames( singleDataSet)[ncol( singleDataSet)] <- survIDname
...
prepareDCdata <- function( DCdat, DCobserverInfo, DCbiasFormula, DCsampleAreaName, DCmethod){
  ...
  for( ss in unique( DCdat[,DCobserverInfo$SurveyID])){
    ...
    tmp[[kount]] <- prepareSingleSurvey( singleDataSet=DCdat[ssID,], datasetID=ss, DCcols=DCcols, survIDname=DCobserverInfo$SurveyID, DCmethod=DCmethod)

  ...
  #  polyNum <- sf::st_intersection( x=nodeLocs, y=domBuff)
    polyNum <- sf::st_intersects( nodeLocs, domBuff, sparse=FALSE)
    ...
    triSumms <- apply( inTris, 1, find1TriInfo, nl=mesh$loc[,1:2])#nodeLocs)
    areas <- triSumms[1,]
  ...
  #  points( nodeLocs[is.na( polyNum),], col='red', pch=3, cex=0.1)
    points( mesh$loc[inPts,1:2], col='darkgreen', cex=0.1)
    points( mesh$loc[ouPts,1:2], col='red', pch=3, cex=0.1) 

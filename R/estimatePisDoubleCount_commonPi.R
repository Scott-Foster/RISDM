...
####	Re-programmed by Scott Dec 2023 to allow for common probability between observers
####
###############################################################################################
###############################################################################################

estimatePisDoubleCount_commonPi <- function( dat){
...
#  start.vals <- cbind( (dat[,1]+dat[,3]), dat[,2]+dat[,3]) / pmax( rowSums( dat, na.rm=TRUE), 1e-4)
  start.vals <- rowMeans( start.vals, na.rm=TRUE)
  ...
  tmp <- stats::optim( par=start.vals, fn=DC_loglikFun_commonPi, dat=dat)

...
\usage{
 makeMesh( ras, max.n=NULL, dep.range=NULL, expans.mult=NULL, hull.res=100, max.edge=NULL, cutoff=NULL, offset=NULL, doPlot=TRUE, ...)
}
\arguments{
...
\item{cutoff}{A numeric vector of 2 elements. The first (second) element describes the mesh building algorithms starting smallest distance between any two nodes within the inner (outer) domain. If NULL (default) a value of 0.2*max.edge is used.}
...
\details{
 ...
 The creation of the outer area is important to ensure that the random spatial effect within the model used by \cite{\link{isdm}} is properly specified and is less likely to exhibit edge effects.
...
\value{
 An extended inla.mesh object describing the mesh on which to perform computing within the region of interest. The extension has two extra list elements: 1) the boundary to the non-NA entries in the raster, from \code{\link{makeBoundary}}; and 2) the nonconvex hull around the non-NA elements.
}

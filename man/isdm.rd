...
\item{ covarBrick}{A RasterBrick (or RasterStack) object containing all the raster information (mostly covariates but see habitatArea argument) in the distribution and bias models.}
\item{ habitatArea}{A character giving the name of a layer of covarBrick that corresponds to the amount of area within each cell of the raster brick that is suitable habitat for the species. This is an area value, units should match those that define the sampling area for the PA, AA, and DC data.}
...
\item{ distributionFormula}{A formula that describes how the species relates to the covariates \emph{but without any potential bias}. Typically, this will include environmental covariates, basis-expanded versions of them (e.g. quadratics) and interactions. Terms in this formula must be present in covarBrick. We note that basis expansions are useful and effective (e.g. poly() orthogonal polynomials), but these terms are standardised by default (away from being orthonormal).}
\item{biasFormula}{A formula describing the bias of the PO data. The default is an intercept-only formula, which encapsulates the assumption that the PO contain no observer-bias -- the pattern varies only with the distribution of the organisms. Terms in this formula must be present in covarBrick. Like the distributionFormula, basis expansions are allowed, but are once again standardised.}

...
%
%<<singleDataPO, eval=TRUE, fig.height=3.5, fig.cap="Intensity model and predictions from estimation using only PO data.", fig.height=3.5>>=
%#PO data only
%#note that ACC is not part of the biasFormula
%#   there is no way it can be estimated in both parts of
%#   the model.
...
%                          int.sd=1000, other.sd=10, prior.mean=0,
%                          prior.range=c(1,0.1), prior.space.sigma=c( 5,0.1)))
%fm.PO$preds <- predict( fm.PO, covars=covars, S=500, 
...
%            control=list( coord.names=c("x","y"), 
%                          int.sd=1000, other.sd=10, prior.mean=0,
%                          prior.range=c(1,0.1), prior.space.sigma=c( 5,0.1), addRandom=FALSE))
%fm.PA$preds <- predict( fm.PA, covars=covars, S=500, 
%                       intercept.terms="PA_Intercept")
